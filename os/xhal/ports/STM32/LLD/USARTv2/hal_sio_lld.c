...
__STATIC_INLINE sioevents_t usart_isr2evt(uint32_t isr) {
...
const SIOConfig *sio_lld_setcfg(SIODriver *siop, const SIOConfig *config) {
 ...
 /* Baud rate setting.*/
  ...
  /* Setting up USART.*/
  u->CR1   = config->cr1 & ~USART_CR1_CFG_FORBIDDEN;
...
void sio_lld_update_enable_flags(SIODriver *siop) {
  ...
  cr1 |= __sio_reloc_field(siop->enabled, SIO_EV_RX_NOTEMPTY,  SIO_EV_RX_NOTEMPTY_POS,  USART_CR1_RXNEIE_Pos) |
         __sio_reloc_field(siop->enabled, SIO_EV_TX_NOTFULL,  SIO_EV_TX_NOTFULL_POS,  USART_CR1_TXEIE_Pos)  |
         __sio_reloc_field(siop->enabled, SIO_EV_RX_IDLE,     SIO_EV_RX_IDLE_POS,     USART_CR1_IDLEIE_Pos) |
  ...
  if (isr != 0U) {
    ...
    if ((isr & SIO_LLD_ISR_RX_ERRORS) != 0U) {
      /* NOTE: We could get here because various causes: PEIE, EIE, LBDIE and
               RXNEIE. Because RXNEIE we can get here even when the
