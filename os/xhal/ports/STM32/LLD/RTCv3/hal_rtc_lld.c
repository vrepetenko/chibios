 ...
 * @brief   STM32 RTCv3 low level driver source.
 *
 * @addtogroup HAL_RTC
 * @{
 */
...
static rtceventflags_t rtc_update_rtc_events(uint32_t cr, uint32_t misr) {
  ...
  if (((cr & RTC_CR_TSIE) != 0U) && ((misr & RTC_MISR_ITSMF) != 0U)) {
    flags |= RTC_FLAGS_TS;
  }
#endif
...
static rtceventflags_t rtc_update_events(uint32_t cr,
                                         uint32_t rtc_misr,
                                         uint32_t tamp_misr) {
  rtceventflags_t flags;
  syssts_t sts;

  flags = rtc_update_rtc_events(cr, rtc_misr) |
          rtc_update_tamp_events(tamp_misr);
...
void rtc_lld_serve_interrupt(void) {
  uint32_t cr, rtc_misr, tamp_misr;
  rtceventflags_t flags;

  cr = RTCD1.rtc->CR;
  rtc_misr = RTCD1.rtc->MISR;
  RTCD1.rtc->SCR = rtc_misr;
  tamp_misr = RTCD1.tamp->MISR;
  RTCD1.tamp->SCR = tamp_misr;
  STM32_RTC_CLEAR_ALL_EXTI();
  flags = rtc_update_events(cr, rtc_misr, tamp_misr);
...
msg_t rtc_lld_start(hal_rtc_driver_c *rtcp) {
  ...
  rtcp->tamp->CR1 = (rtcp->tamp->CR1 & ~STM32_TAMP_CR1_MASK) |
                    (STM32_TAMP_CR1_INIT & STM32_TAMP_CR1_MASK);
  rtcp->tamp->CR2 = (rtcp->tamp->CR2 & ~STM32_TAMP_CR2_MASK) |
                    (STM32_TAMP_CR2_INIT & STM32_TAMP_CR2_MASK);
  rtcp->tamp->FLTCR = (rtcp->tamp->FLTCR & ~STM32_TAMP_FLTCR_MASK) |
                      (STM32_TAMP_FLTCR_INIT & STM32_TAMP_FLTCR_MASK);
  rtcp->tamp->IER = (rtcp->tamp->IER & ~STM32_TAMP_IER_MASK) |
                    (STM32_TAMP_IER_INIT & STM32_TAMP_IER_MASK);
  rtc_wpr_lock(rtcp);
